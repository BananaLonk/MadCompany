...
import gioco.oggetti.oggettiMappa.Oggetto;
import gioco.oggetti.oggettiMappa.RimuoviLivello;
import gioco.oggetti.oggettiMappa.Solido;
import gioco.oggetti.oggettiMappa.Teletrasporto;
import gioco.personaggi.*;

import java.io.FileNotFoundException;

/**
 * mappa iniziale del gioco, prima del rapimento iniziale
 */
public class OpenWorld extends BaseScreen{
    ...
    private float contatoreDialoghi =0;
    ...
    public void initialize() {
        ...
        tma = new TilemapActor("assets/mappa iniziale.tmx", mainStage, camera);
        tma.setScale(4);
        ...
        for (BaseActor actor : BaseActor.getList(mainStage, Oggetto.class.getName()))
        {
            ...
            if(actor instanceof Teletrasporto){
                if ( Squadra.joker.overlaps(actor))
                {
                    ...
                    if(((Teletrasporto) actor).getDestinazione().equalsIgnoreCase("puzzle")){
                        Launcher.dungeon=new Dungeon();
                        Launcher.cambiaMappa(Launcher.dungeon);
                    }
                    if(((Teletrasporto) actor).getDestinazione().equalsIgnoreCase("youtopia")){
                        Launcher.youtopia=new Youtopia();
                        Launcher.cambiaMappa(Launcher.youtopia);
                    }
                }
            }
    ...
    private void controllaDialoghi(float dt){
        if (BaseActor.getList(mainStage,Dialogo.class.getName()).isEmpty()) {
            contatoreDialoghi +=dt;
            if(contatoreDialoghi >15){
                contatoreDialoghi =0;
                try {
