...
/**
 * mappa futuristica del gioco, permette di accedere al dungeon
 */
public class Youtopia extends BaseScreen{
    ...
    private boolean tuttiSconfitti;
    ...
    public void initialize() {
        ...
        try {
            // Prova a caricare la mappa senza altri parametri
            TiledMap map = new TmxMapLoader().load("assets/newyoutopia.tmx");
            Gdx.app.log("Youtopia", "Mappa caricata con successo!");

            // Poi prova a creare il TilemapActor
            tma = new TilemapActor("assets/newyoutopia.tmx", mainStage, camera);
            tma.setScale(4);
        } catch (Exception e) {
            Gdx.app.error("Youtopia", "Errore nel caricamento mappa", e);
        }
        //carico la mappa e gli oggetti di tutti i tipia
        tma.setScale(4);
        ...
        {
            MapProperties props = obj.getProperties();
            new Nemico((float) props.get("x"), (float) props.get("y"),mainStage, (String) props.get("percorso"));
        }
        //carico la porta
        ...
        //resize (non funziona e non so come farlo funzionare)
    ...
    public void update(float dt) {
        ...
        if(fade){
            ...
            if(fadeAlpha>=1){
                ...
                Launcher.cambiaMappa(new Battaglia(null));
            }
        }

        //controllo se i nemici sono stati sconfitti
        tuttiSconfitti=true;
        for(BaseActor actor : BaseActor.getList(mainStage, Nemico.class.getName())) {
            if (!((Nemico) actor).isSconfitto()) {
                tuttiSconfitti = false;
                break;
            }
        }
        if(tuttiSconfitti){
            Porta porta=(Porta)(BaseActor.getList(mainStage, Porta.class.getName()).get(0));
            if(porta.isEnable())
                Gdx.audio.newSound(Gdx.files.internal("sounds/portaAperta.mp3")).play();
            porta.setEnable(false);
        }
        ...
        if(Gdx.input.isKeyJustPressed(Input.Keys.K)){
            for(BaseActor actor : BaseActor.getList(mainStage, Nemico.class.getName())){
                ((Nemico) actor).setSconfitto(true);
            }
        }
    ...
    /**
     * renderizza il menu visualizzato premendo ESC (non esiste ancora)
     */
    ...
    /**
     * permette di visualizzare i contorni delle hitbox di tutti gli oggetti presenti
     */
    ...
    /**
     * metodo chiamato ad ogni frame che gestisce il comportamento degli oggetti caricati dalla mappa, come teletrasporti o collisioni
     */
    private void controllaOggetti(){
        ...
        for (BaseActor actor : BaseActor.getList(mainStage, Oggetto.class.getName()))
        {
            ...
            if(actor instanceof Teletrasporto){
                if ( Squadra.joker.overlaps(actor))
                {
                    ...
                    if(((Teletrasporto) actor).getDestinazione().equalsIgnoreCase("shop")){
                        Launcher.shopYoutopia=new ShopYoutopia();
                        Launcher.cambiaMappa(Launcher.shopYoutopia);
                    }
                    ...
                    if(((Teletrasporto) actor).getDestinazione().equalsIgnoreCase("dungeon")){
                        Launcher.dungeon=new Dungeon();
                        Launcher.cambiaMappa(Launcher.dungeon);
                    }
                }
            }
            if(actor instanceof RimuoviLivello){
                if((compagniOvrelap(actor) || Squadra.joker.overlaps(actor)) && ((RimuoviLivello) actor).isEnable()){
        ...
        for(BaseActor nemico: BaseActor.getList(mainStage,Nemico.class.getName())) {
            if (Squadra.joker.overlaps(nemico)) {
                fade=true;
                ((Nemico)nemico).setSconfitto(true);
            }
        }
    }

    /**
     * renderizza il layer normalmente nascosto della mappa
     */
    ...
    /**
     * controlla se le hitbox dei compagni si sovrappongono all'oggetto passato
     * @param oggetto oggetto di cui controllare le collisioni
     * @return true se almeno un compagno è sovrapposto
     */
